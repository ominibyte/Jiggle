...
/*
 * This class goes through all the XML files generated from the CodeSmellFinderMin based on modified ruleset properties 
 * and populates the MySQL database for future queries. The CodeSmell ruleset of focus is just CodeSize.
 */
public class CodeSmellMinXMLParser {
	private JiggleActivityReporter reporter;
	private static final File XML_FOLDER_ROOT = CodeSmellFinderMin.CODE_SMELLS_FOLDER;
	
	public CodeSmellMinXMLParser(JiggleActivityReporter reporter){
			...
			try{
				...
				if( doc.getDocumentElement().hasChildNodes() ){
					...
					for (int i = 0; i < fileNodeList.getLength(); i++) {
						...
						Query q = new Query("INSERT IGNORE INTO new_code_smell VALUES (NULL, ?, ?)", Util.mcon);
						...
						for(int j = 0; j < violationNodeList.getLength(); j++){
							...
							//insert the rule
							q = new Query("INSERT IGNORE INTO new_code_smell_rules VALUES (NULL, ?)", Util.mcon);
							if( q.isFullyConnected() ){
								PreparedStatement ps = q.getPS();
								ps.setString(1, rule);
							...
							//insert the ruleset
							q = new Query("INSERT IGNORE INTO new_code_smell_ruleset VALUES (NULL, ?)", Util.mcon);
							if( q.isFullyConnected() ){
								PreparedStatement ps = q.getPS();
							...
							q = new Query("INSERT INTO new_code_smell_violation VALUES (NULL, (SELECT id FROM code_smell WHERE project_id = ? AND file = ?), ?, ?, ?, ?, (SELECT id FROM code_smell_rules WHERE rule = ?), (SELECT id FROM code_smell_ruleset WHERE ruleset = ?), ?, ?, ?, ?, ?)", Util.mcon);
							if( q.isFullyConnected() ){
								PreparedStatement ps = q.getPS();
								int pos = 0;
	...
	public static void main(String[] args){
		new CodeSmellMinXMLParser(null);
	}
}
